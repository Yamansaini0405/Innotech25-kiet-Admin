...
export default function AssignJudgePage() {
  ...
  const fetchTeams = async () => {
    if (!selectedDept || !selectedCategory) {
      setMessage({ type: "error", text: "Please select both department and category" })
      return
    }
    ...
    try {
      ...
      const response = await fetch(
        `${baseUrl}/api/admin/getbydepartmentandcategory?department=${selectedDept}&categoryId=${selectedCategory}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleAssignJudges = async () => {
    ...
    try {
      ...
      if (data.success) {
        // Use the 'data.message' from the backend instead of 'data.data.length'
                ...
                <tbody className="divide-y divide-slate-200">
                  {teams.map((team) => (
                    <tr key={team.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm text-slate-600">{team.categoryId}</td>
